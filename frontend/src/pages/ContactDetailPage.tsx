...
export default function ContactDetailPage() {
  ...
  if (!contact) return <div className="text-slate-500 animate-pulse">Загрузка...</div>;
  ...
  return (
    <div className="space-y-4 animate-in max-w-4xl">
      <button onClick={() => navigate('/contacts')} className="flex items-center gap-1 text-slate-500 hover:text-white text-sm transition mb-2">
        <ArrowLeft className="w-4 h-4" /> Контакты
      </button>
      ...
      <div className="card p-5">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-3">
              ...
              <span className={`badge ${statusColor(contact.status)}`}>{{new:'Новый',active:'Активный',inactive:'Неактивный',lost:'Потерян'}[contact.status]||contact.status}</span>
            </div>
            ...
            {contact.source && <div className="text-xs text-slate-600 mt-1">Источник: {contact.source}</div>}
          </div>
        ...
        <div className="text-xs text-slate-600 mt-3">Добавлен {formatDate(contact.created_at)}</div>
      </div>
      ...
      <div className="flex gap-1 border-b border-slate-700">
        {(['activities', 'calls', 'messages'] as const).map(t => (
          ...
          >{{activities:'Активности',calls:'Звонки',messages:'Сообщения'}[t]||t}</button>
        ))}
      </div>
      ...
      {tab === 'activities' && (
        <div className="space-y-2">
          {activities.length === 0 ? (
            <div className="text-slate-500 text-sm py-6 text-center">Активностей нет</div>
          ) : activities.map(a => (
      ...
      {tab === 'calls' && (
        <div className="space-y-2">
          {calls.length === 0 ? (
            <div className="text-slate-500 text-sm py-6 text-center">Звонков нет</div>
          ) : calls.map(c => (
      ...
      {tab === 'messages' && (
        <div className="text-slate-500 text-sm py-6 text-center">
          ...
          Сообщения WhatsApp появятся здесь
        </div>
      )}
