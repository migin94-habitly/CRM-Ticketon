...
export default function AnalyticsPage() {
  ...
  if (loading) return <div className="text-slate-500 animate-pulse">Loading analytics...</div>;
  if (!metrics) return <div className="text-slate-500">Failed to load</div>;
    ...
    <div className="space-y-6 animate-in">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm">Deep performance insights</p>
        </div>
      ...
      <div className="card p-5 border border-primary-500/30 bg-primary-500/5">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="font-semibold text-white">AI Sales Intelligence</h3>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="stat-card">
          ...
          <div className="text-xs text-slate-500">Total Won Revenue</div>
          <div className="text-xs text-green-400">{metrics.won_deals} deals closed</div>
        </div>
        <div className="stat-card">
          ...
          <div className="text-xs text-slate-500">{metrics.total_deals} total deals</div>
        </div>
        <div className="stat-card">
          <BarChart3 className="w-5 h-5 text-blue-400" />
          <div className="text-2xl font-bold text-white">{formatCurrency(metrics.avg_deal_value)}</div>
          <div className="text-xs text-slate-500">Avg Deal Value</div>
          <div className="text-xs text-slate-500">Pipeline: {formatCurrency(metrics.total_value)}</div>
        </div>
        <div className="stat-card">
          ...
          <div className="text-xs text-slate-500">{metrics.total_messages} messages</div>
        </div>
      </div>
      ...
      {forecast.length > 0 && (
        <div className="card p-5">
          <h3 className="font-semibold text-white mb-4 flex items-center gap-2">
            <Zap className="w-4 h-4 text-primary-400" /> AI Sales Forecast (Weighted Pipeline)
          </h3>
          <ResponsiveContainer width="100%" height={220}>
            <ComposedChart data={forecast}>
              ...
              <Bar dataKey="pipeline" fill="#6366f1" opacity={0.5} radius={[4,4,0,0]} name="Pipeline" />
              <Line type="monotone" dataKey="forecast" stroke="#10b981" strokeWidth={2} dot={{ fill: '#10b981', r: 3 }} name="AI Forecast" />
            </ComposedChart>
          </ResponsiveContainer>
          <p className="text-xs text-slate-600 mt-2">Forecast = pipeline × win probability</p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="card p-5">
          <h3 className="font-semibold text-white mb-4">Sales Rep Performance</h3>
